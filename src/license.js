...
/** 与 settings 中 cursorMcp.licenseSecret 一致；留空时使用（请在 settings 中配置自己的密钥） */
exports.DEFAULT_LICENSE_SECRET = "";
exports.GLOBAL_STATE_LICENSE_KEY = "cursorMcp.license.v1";
exports.GLOBAL_STATE_USED_NONCES_KEY = "cursorMcp.usedLicenseNonces.v1";
exports.GLOBAL_STATE_TRIAL_UNTIL_KEY = "cursorMcp.trialUntil.v1";
exports.GLOBAL_STATE_TRIAL_USED_KEY = "cursorMcp.trialUsed.v1";
/** 试用时长（每机仅一次） */
...
function verifyAndParseToken(secret, token) {
    ...
    if (parts.length !== 3 || parts[0] !== "CMC1") {
        return { ok: false, error: "卡密格式不正确（应以 CMC1. 开头）" };
    }
    let payload;
    try {
    ...
    if (!crypto.timingSafeEqual(sigBuf, expected)) {
        return { ok: false, error: "卡密无效或与当前签名密钥不匹配（请检查设置 cursorMcp.licenseSecret）" };
    }
    if (dur === "timed") {
        if (durationMs === undefined) {
...
/**
 * 激活：以 `CMC1.` 开头的走本地 HMAC 卡密；否则请求云端 `redeemApiBaseUrl` + `/api/redeem` 核销后写入本机授权（永久）。
 */
async function tryActivateLicenseAsync(context, keyInput) {
    ...
    const cfg = vscode.workspace.getConfiguration("cursorMcp");
    if (key.startsWith("CMC1.")) {
        const cloudOnly = cfg.get("cloudLicenseOnly") !== false;
        if (cloudOnly) {
            return {
                ok: false,
                msg: "当前为仅云端卡密模式：请使用发卡系统生成的卡密格式（可在设置中将 cursorMcp.cloudLicenseOnly 设为 false 以使用 CMC1 本地卡密）。",
            };
        }
    ...
    if (!baseUrl) {
        return {
            ok: false,
            msg: "未配置云端核销地址：请在设置中填写 cursorMcp.redeemApiBaseUrl（您自己的发卡服务地址）",
        };
    }
