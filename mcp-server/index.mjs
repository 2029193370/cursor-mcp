...
// 多会话：CURSOR_MCP_SESSION=1…32 时队列为 ~/.cursor/cursor-mcp-messages/s/<id>/
// 未设置环境变量时保持旧路径（兼容单实例 cursor-mcp）
import { homedir } from "os";
const queueRoot = join(homedir(), ".cursor", "cursor-mcp-messages");
const sessionKey = (process.env.CURSOR_MCP_SESSION || "").trim();
...
function buildSystemSuffix() {
  ...
  if (sessionKey) {
    suffix += `【会话通道】MCP-${sessionKey}：本进程仅处理插件发到该通道的消息；请在本对话中调用 cursor-mcp-${sessionKey} 的 check_messages。\n\n`;
  }
...
const serverName = sessionKey ? `cursor-mcp-${sessionKey}` : "cursor-mcp";
